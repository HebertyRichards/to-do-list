...
function frequencyLabel(habit: Habit): string {
  if (habit.every_day) return "Todos os dias";
  const set = new Set(habit.days_of_week);
  const labels = DAYS_OF_WEEK.filter((d) => set.has(d.value)).map((d) => d.short);
  return labels.length ? labels.join(", ") : "Nenhum dia";
}
